"""Module gets the stocks based on filtered URL from finviz"""
import json
import pathlib
...
import autoviz.helper_functions.scraper_functions as scrape
from autoviz.helper_functions.display_functions import create_table_string
from autoviz.helper_functions.error_handling import InvalidTableType, NoResults
from autoviz.helper_functions.request_functions import (Connector,
                                                        http_request_get,
                                                        sequential_data_scrape)
from autoviz.helper_functions.save_data import export_to_csv, export_to_db
...
class Screener(object):
    ...
    def __check_table(input_table):
        """ Checks if the user input for table type is correct.
        Otherwise, raises an InvalidTableType error.
        """
    ...
    def load_filter_dict(reload=True):
        """
        Get dict of available filters. File containing json
        specification of filters will be built if it doesn't exist
    ...
    def __search_screener(self):
        ...
        if self._request_method == "async":
            async_connector = Connector(
                scrape.get_table,
                scrape.get_page_urls(self._page_content,
                                     self._rows, self._url),
                self._user_agent,
                self.headers,
                self._rows,
                css_select=True,
